...
test("field mapping updates validate Banner fields and audit the acting user", async () => {
  ...
  assert.match(source, /const maxFallbackValueLength = 2000/);
  assert.match(source, /fallbackValue: z\.string\(\)\.trim\(\)\.max\(maxFallbackValueLength\)\.optional\(\)/);
...
test("field mapping updates bound and normalize fallback values", async () => {
  ...
  assert.match(source, /const maxFallbackValueLength = 2000/);
  assert.match(source, /fallbackValue: z\.string\(\)\.trim\(\)\.max\(maxFallbackValueLength\)\.optional\(\)/);
  assert.match(source, /mapping\.fallbackValue \|\| null/);
  assert.match(checklist, /Field mapping fallback values are limited to 2,000 characters/);
});
